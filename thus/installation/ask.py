...
class InstallationAsk(GtkBaseBox):
    ...
    def translate_ui(self):
        ...
        if len(oses_str) > 0:
            txt = _("Replace {0} with Manjaro").format(oses_str)
        else:
            txt = _("Erase disk and install Manjaro")
        radio.set_label(txt)
        ...
        # if len(oses_str) > 0:
        #     txt = _("Install Manjaro alongside {0}").format(oses_str)
        #     radio = self.ui.get_object("alongside_radiobutton")
        #     radio.set_label(txt)
        #
        #     label = self.ui.get_object("alongside_description")
        #     txt = _("Installs Manjaro without removing {0}").format(oses_str)
        #     txt = description_style.format(txt)
        ...
        radio.set_label(_("Choose exactly where Manjaro should be installed."))
    ...
    def store_values(self):
        ...
        if self.next_page == "installation_automatic":
            ...
            self.settings.set('luks_root_volume', "cryptManjaro")
