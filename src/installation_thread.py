...
class InstallationThread(threading.Thread):
    ...
    def run(self):
        ...
        self.source_desktop_dir = "/source_desktop"
        if not os.path.exists(self.source_desktop_dir):
            os.makedirs(self.source_desktop_dir)
        ...
        self.kernel_pkg = self.settings.get("KERNEL")
        self.vmlinuz = "vmlinuz-%s-%s" % (self.settings.get("KERNR"), self.arch)
        self.initramfs = "initramfs-%s-%s" % (self.settings.get("KERNR"), self.arch)
        ...
        try:
            self.queue_event('debug', 'Unsquashing files...')
            self.copy_sqfs()
    ...
    def step_mount_source(self):
        ...
        self.media = "/bootmnt/manjaro/%s/%s.sqfs" % (self.arch, self.settings.get("ROOT_SQF"))
        self.media_desktop = "/bootmnt/manjaro/%s/%s.sqfs" % (self.arch, self.settings.get("DE_SQF"))
        self.queue_event('debug', "Mounting %s on %s" % (self.media, "/source/"))
        self.do_mount(self.media, "/source/", "squashfs", options="loop")
        self.queue_event('debug', "Mounting %s on %s" % (self.media_desktop, "/source_desktop/"))
        self.do_mount(self.media_desktop, "/source_desktop/", "squashfs", options="loop")

    # copy sqfs images to target
    def copy_sqfs(self):
        self.step_mount_source()
        self.queue_event('debug', "Coping sqfs-images to /install")
        ...
        os.chdir(SOURCE)
        # index the files
        self.queue_event('debug', "Indexing files to be copied..")
        for top,dirs,files in os.walk(SOURCE, topdown=False):
            our_total += len(dirs) + len(files)            
        our_total += 1 # safenessness
        self.queue_event('debug', "Copying desktop files")
        for top,dirs,files in os.walk(SOURCE):
            # Sanity check. Python is a bit schitzo
            dirpath = top
            if(dirpath.startswith(SOURCE)):
            ...
            # Apply timestamps to all directories now that the items within them
            # have been copied.
        self.queue_event('debug', "Restoring meta-info")
        for dirtime in directory_times:
            (directory, atime, mtime) = dirtime
            try:
                self.queue_event('debug', "Restoring meta-information on %s" % directory)
    ...
    def configure_system(self):
        ...
        # enable services      
        self.enable_services([ "lxdm", "NetworkManager" ])
        ...
        self.chroot(['useradd', '-m', '-s', '/bin/bash', \
                  '-g', 'users', '-G', 'video,audio,power,disk,storage,optical,network,lp,scanner,wheel', \
                  username])
